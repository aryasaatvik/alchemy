...
// The collector combinators are typed against @distilled.cloud/otel-collector.
// tsgo runs without the \`bun\` condition, so this resolves the package's
// \`types\` condition (\`lib/**/*.d.ts\`) — the declarations a Node consumer sees.
// A tarball that ships \`src\` but no \`lib\` fails here, not in production.
export const collectorConfig = AWS.Lambda.collector({
  ...
  pipelines: {
    traces: AWS.Lambda.pipeline({
      ...
      processors: [AWS.Lambda.Processor.batch({ timeout: Duration.seconds(1) })],
      exporters: [
        AWS.Lambda.Exporter.otlpHttp({
          endpoint: "https://api.example.invalid",
          headers: { authorization: "Bearer \${env:ACCEPTANCE_TOKEN}" },
        }),
      ],
    }),
  },
...
// \`AWS.Lambda.collector\` compiles its components through
// @distilled.cloud/otel-collector's generated codecs. Emitting a config here
// proves the package resolves and runs from the tarball — the required-archive
// check above only proves the built entry is present.
  ...
  pipelines: {
    traces: pipeline({
      ...
      exporters: [
        Exporter.otlpHttp({
          endpoint: "https://api.example.invalid",
          headers: { authorization: Redacted.make("acceptance-token") },
        }),
      ],
    }),
  },
...
const exporterHeader =
  parsed.exporters?.otlphttp?.headers?.authorization;
const placeholders = Object.keys(emitted.env);
if (
  parsed.receivers?.otlp?.protocols?.http?.endpoint !== "127.0.0.1:4318" ||
  parsed.processors?.batch?.timeout !== "1s" ||
  parsed.service?.pipelines?.traces?.exporters?.[0] !== "otlphttp" ||
  placeholders.length !== 1 ||
  exporterHeader !== \`\\\${env:\${placeholders[0]}}\` ||
  emitted.content.includes("acceptance-token")
) {
  throw new Error("Collector configuration did not emit through the packed codecs");
}

console.log(
  "fresh-consumer Lambda bundle, runtime fixture, local Worker binding, and collector config passed",
);
`,
  );
...
try {
  ...
  for (const { directory, manifest: originalManifest } of closure.values()) {
    ...
    // The Vite/Rolldown plugins are loaded through Alchemy's self-resolver
    // while bundling a Lambda whose graph also reaches Worker infrastructure.
    // Their runtime exports must be resolvable from the installed archive.
    if (name === "@alchemy.run/cloudflare-runtime") {
  ...
  for (const workspace of packed.values()) {
    ...
    for (const section of dependencySections) {
      ...
      for (const [name, originalSpecifier] of Object.entries(
        ...
        // Bun emits `catalog:` peers that resolve to a workspace package as a
        // verbatim `workspace:*` in the packed manifest, so gate on the packed
        // specifier as well as the authored one.
        const packedSpecifier = packedDependencies[name];
        const isWorkspaceRef =
          originalSpecifier.startsWith("workspace:") ||
          (typeof packedSpecifier === "string" &&
            packedSpecifier.startsWith("workspace:"));
        if (!isWorkspaceRef) continue;
  ...
  for (const requiredRuntimeEntry of [
    ...
    // `AWS.Lambda.collector` resolves the pinned collector build through this
    // subpath. Its `bun` condition points at `src/`, so a missing `lib/` only
    // surfaces under the `default`/`types` conditions — i.e. in the consumer,
    // not here. Assert the built entry ships.
