...
/**
 * The exact Effect version the workspace pins through `overrides`. A fresh
 * consumer installs this beside the artifact so it resolves the same Effect
 * identity the closure was built and verified against.
 */
export const workspaceEffectVersion = async (
  repositoryRoot: string,
): Promise<string> => {
  const version = (await readPnpmWorkspace(repositoryRoot)).overrides?.effect;
  if (version === undefined || !/^\d+\.\d+\.\d+(-[\w.]+)?$/.test(version))
    throw new Error(
      `pnpm-workspace.yaml must pin overrides.effect to an exact version; found ${version}`,
    );
  return version;
};
