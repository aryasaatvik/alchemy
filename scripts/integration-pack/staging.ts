...
/**
 * Subpaths whose export targets do not exist at this base. The staged
 * manifest drops them only while they stay missing, so an upstream fix
 * reinstates them automatically.
 */
const knownMissingExports: Readonly<Record<string, ReadonlyArray<string>>> = {
  ...
  // Distilled's core package declares a root export without src/index.ts.
  // Consumers use the supported subpaths.
...
export const stageAndPack = async (
  ...
  try {
    ...
    // pnpm 11 reads overrides only from pnpm-workspace.yaml. Without them a
    // transitive edge (e.g. @distilled.cloud/aws -> @distilled.cloud/core
    // at the same version string) resolves from the registry, and hoisting
    // can bundle that published copy instead of the local build.
    ...
    const workspaceSettings = join(directory, "pnpm-workspace.yaml");
    await writeFile(workspaceSettings, `${JSON.stringify({ overrides })}\n`);
    await run(pnpmInstallCommand(input.pnpmVersion), { cwd: directory });
    await rm(workspaceSettings);
    ...
    await run(pnpmPackCommand(input.pnpmVersion, destination), {
      cwd: directory,
    });
