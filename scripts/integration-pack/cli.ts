...
export const main = async (): Promise<void> => {
  ...
  for (const workspace of workspaces) {
    ...
    if (workspace.manifest.scripts?.build !== undefined) {
      await run(["bun", "run", "build"], { cwd: workspace.directory });
    ...
    const version = isEntrypoint
      ...
      : (workspace.manifest.version ?? "0.0.0");
    const localPackages = [...packed.values()];
    // Every already-packed local package is a direct staging dependency. This
    // lets Bun satisfy workspace edges found inside a tarball without giving
    // the publish manifest any file: dependency.
