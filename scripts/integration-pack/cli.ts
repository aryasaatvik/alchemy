...
import { repositoryPnpmVersion } from "./pack.ts";
...
/** Tracked-file state; a build that changes it invalidates the fingerprint. */
...
export const main = async (): Promise<void> => {
  ...
  const statusBefore = await trackedStatus();
  ...
  const pnpmVersion = await repositoryPnpmVersion(repositoryRoot);
  ...
  for (const workspace of workspaces) {
    ...
    if (workspace.manifest.scripts?.build !== undefined) {
      ...
      // The fingerprint was computed from the tree before this build. A build
      // that rewrites tracked inputs (e.g. a generated manifest) would ship
      // content the version does not identify.
      if ((await trackedStatus()) !== statusBefore) {
        throw new Error(
          `Building ${workspace.name} modified tracked files; commit the regenerated output and pack again:\n${await trackedStatus()}`,
        );
      }
  ...
  if (options.verify === "consumer")
    await verifyFreshConsumer(artifact, {
      effect: await workspaceEffectVersion(repositoryRoot),
    });
