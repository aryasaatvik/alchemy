...
export const hydrateArtifactsRepository = ({
  ...
  createToken: async (scope, ttl) =>
    JSON.parse(await methods.createToken(scope, ttl)),
...
export class ArtifactsBindingProxy extends RpcTarget {
  ...
  async import(params: Parameters<Artifacts["import"]>[0]): Promise<string> {
    return JSON.stringify(await this.#binding.import(params));
  }

  async list(options?: Parameters<Artifacts["list"]>[0]): Promise<string> {
