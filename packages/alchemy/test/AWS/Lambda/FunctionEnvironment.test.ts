...
describe("Lambda environment size", () => {
  ...
  it("accepts exactly 4 KiB and rejects the next byte", () => {
    ...
    expect(error.entryCount).toBe(1);
  ...
  it("reports enough key-only contributors to diagnose aggregate overages", () => {
    const environment = Object.fromEntries(
      Array.from({ length: 30 }, (_, index) => [
        `CONFIG_${String(index).padStart(2, "0")}`,
        "x".repeat(140),
      ]),
    );
    const error = Effect.runSync(
    ...
    expect(error.entryCount).toBe(30);
    expect(error.largestEntries).toHaveLength(20);
    expect(error.message).not.toContain("x".repeat(140));
  });
