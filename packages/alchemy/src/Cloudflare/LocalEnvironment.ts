...
export class LiveCloudflareEnvironment extends Context.Service<
  ...
  Effect.Effect<CloudflareEnvironment.CloudflareEnvironmentShape>
...
export const retainedLiveEnvironment = Layer.effect(
  ...
  Effect.gen(function* () {
    const context = yield* Effect.context<
      ProfileStore | AuthProviders | CredentialsStore
    >();
    return liveEnvironmentFromProfile.pipe(
      Effect.provide(context),
      Effect.orDie,
    ) as Effect.Effect<CloudflareEnvironment.CloudflareEnvironmentShape>;
  }),
);
...
export const resolveLiveEnvironment: Effect.Effect<CloudflareEnvironment.CloudflareEnvironmentShape> =
  Effect.gen(function* () {
    ...
    return yield* Option.match(environment, {
      ...
      onSome: (environment) =>
        environment as Effect.Effect<CloudflareEnvironment.CloudflareEnvironmentShape>,
    });
  });
