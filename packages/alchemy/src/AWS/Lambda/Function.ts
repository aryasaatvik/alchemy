...
/**
 * Resolved Rolldown options shared by deploy builds and the Live Lambda
 * watcher. Keeping one resolver prevents local execution from drifting from
 * the artifact that a normal deploy would produce.
 */
export const resolveFunctionBundleConfig = Effect.fn(function* (
  ...
  // `pure` and `bundleAnalyzer` configure Alchemy's built-in rolldown plugins
  // (`Bundle.BundleExtraOptions`), not rolldown itself — they are threaded
  // through as `extra` and must not leak into `rolldown.InputOptions`.
  const {
    output: buildOutput,
    install,
    pure: _pure,
  ...
  const external = (
    ...
    // import of workerd's virtual module. A Lambda can never resolve it; keep
    // the import external so the runtime fallback handles it without a noisy
    // unresolved-import warning before DCE.
    if (moduleId === "cloudflare:workers") {
      return true;
    }
    if (
...
`,
  ...
  return {
    inputOptions,
    outputOptions,
    // Built-in plugin configuration (`pure`, `bundleAnalyzer`). Every caller
    // — the deploy build and the Live Lambda watcher — must pass this as the
    // third argument to `Bundle.build`/`Bundle.watch`, otherwise local dev
    // tree-shakes differently from the deployed artifact.
    extra: props.build,
    cwd,
    architecture,
    requested,
...
/**
 * Code artifact produced for a Function reconcile.
 *
 * `identityHash` is cheap and drives the provider diff. The archive build is
 * deferred until the provider actually needs to upload code.
 */
...
export interface FunctionProviderOptions {
  /**
   * Substitute the code artifact deployed by the provider.
   *
   * Live Lambda uses this seam to deploy its bridge while reusing the normal
   * Function provider for IAM, bindings, URLs, event sources, and cleanup.
   */
  bundleCode?: (
    id: string,
    props: FunctionProps,
...
export const makeFunctionProvider = (options?: FunctionProviderOptions) =>
  Effect.gen(function* () {
    ...
    return {
      stables: ["functionArn", "functionName", "roleName"] as Array<
      ...
      precreate: Effect.fn(function* ({
        ...
        yield* createOrUpdateFunction({
          id,
          news,
          // `layers` is deliberately omitted: `news` is unresolved here, so
          // a `LayerVersion`'s ARN is still an Output, and layers bound
          // through the binding channel are not even collected until
          // `reconcile` runs `attachBindings`. The stub needs no layers;
          // reconcile attaches the merged list.
      ...
      tail: ({ output }: FunctionLifecycleInput<"tail">) => {
      ...
      logs: ({ output, options }: FunctionLifecycleInput<"logs">) =>
        logs
          .filterLogEvents({
