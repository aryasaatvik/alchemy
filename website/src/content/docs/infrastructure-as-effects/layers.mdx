...
A host's bindings are built for that host. At plan, each Worker or Function builds
the Layers its init provides for itself, so every host that provides
`JobServiceKV` gets its own binding, including a host declared inside
another host's init. Provide a binding Layer in the init of each host that
uses it; a Layer built outside any host's init has no host to bind to.
