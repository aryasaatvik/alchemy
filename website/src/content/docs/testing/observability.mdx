...
Lambda has no `waitUntil`. Direct remote export therefore delays the
handler response; production Lambda functions should flush to a
localhost [Collector
extension](/infrastructure-as-effects/telemetry#lambda-export-through-a-collector-extension)
and let the extension perform the remote export.
